...
7. They toggle the `$team` relation chip off, then back on.
   - *Expected*: the same drop from four edges to three and back, this time removing only the
     ownership edge.
8. They set the toolbar's Team filter to the throwaway team.
...
9. They collapse p1 via its own fold toggle, then expand it again.
...
10. They switch the layout to Manual and drag p1 to a new canvas position.
...
11. They reload the page.
    - *Expected*: Manual stays selected and p1's dragged position is restored — both from the
      ...
      response.
12. Cleanup (API): the two children, then p1, then p2 (p1 is the referrer, deleted before its
...
- **The full filter/graph-endpoint rule table** (the `q` substring filter, unknown-identifier
  ...
  `EntityTest`; the journey proves the blueprint pills narrow the shown workspace.
