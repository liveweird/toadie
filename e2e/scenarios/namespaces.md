...
1. The admin signs in and opens **Namespaces** from the nav.
   - *Expected*: the document editor renders with **Save** disabled (nothing dirty yet).
...
5. A throwaway regular user (created via the one-time reveal flow) signs in and opens
   **Namespaces** from the nav.
   - *Expected*: the read-only numbered list (the `b` entry visible) with no **Add namespace**
     and no **Save** — editing is ADMIN-only.
