...
1. The administrator creates a disposable regular user, then opens **Integration clients** and
   creates a uniquely named client.
   - *Expected*: the create response is `201`; the returned key has the `toadie_int_` prefix and
     the one-time panel reveals that same key only after **Show password** is used.
...
4. The administrator returns, opens the final list page containing the newly created client, and
   confirms **Revoke**.
