...
The stub's `onInit` hands the page a fake instance whose `fitView` records the node ids the stub holds at call time; the `Graph`/`EntityGraph` re-fit cases assert the fit after an expand saw the re-added node (v2.2.1 — the fit must be keyed off the live `nodes`, not `baseLayout`).
...
**Runtime OpenAPI conformance.** Every `/api/` interaction the server test suite produces is validated against `documentation.yaml` by the `OpenApiConformance` Ktor client plugin (`server/src/test/kotlin/OpenApiConformance.kt`), installed via the shared `toadieTestClientDefaults()` — so `jsonClient()`/`authedClient()` traffic is checked automatically and drift (undeclared endpoint/method/status, response-schema or content-type mismatch) fails the exercising test with the validation report. Response side is fully validated; request-side validation is ignored except unknown-path/method (tests deliberately send malformed payloads and missing tokens). The published OpenAPI **3.0.3** spec is fed to the validator verbatim, exactly as external consumers receive it; The validator injects `additionalProperties: false` into every schema (strictness: an undeclared response member fails), which is why the three findings-bearing problem bodies REPEAT `ProblemDetail`'s members instead of composing them: an `allOf` is validated per branch, so each branch rejects the other's members, and the library's `withResolveCombinators(true)` merges branches but drops `nullable` (2.3.1 tried both — keep the copies). `OpenApiSpecTest` pins the version, nullable-response validation, and that the document stays 3.0-compatible (use `nullable:`, never `type: [..., "null"]`) plus static invariants (unique operationIds, 401/500 declared, paths under `/api/v1/`). `-Dopenapi.conformance=warn|off` relaxes enforcement for drift triage only (default `fail`). A coverage report (exercised vs. declared operation/status pairs) is written to `server/build/reports/openapi-conformance/coverage.md` after each test run — a report, not a gate; the one gap class left deliberately unexercised is every operation's declared `500` (the public API offers no honest way to force one — don't chase them). Tests that use `testApplication`'s default `client` bypass the plugin — prefer `jsonClient()`. `NegativeIdTest` GETs/DELETEs `-1` on every by-id operation so the pre-routing negative-id `400` stays declared on each (the conformance plugin fails an undeclared status).
...
(76 entities, the catalog landscape re-told in the baseline ontology): it loads the blueprint
...
(extracted from `Graph.test.tsx`) that both the Backstage `Graph`/`Hierarchy` page
...
pin: the fifteen blueprint files POSTed as ONE `/blueprints/import` batch with
`replaceExisting = true` answer `_team`/`_user` → `UPDATED` and the other thirteen → `CREATED`, with
every re-GET definition matching the two-pass `SampleData.loadBlueprints` shape; the 76 entity
...
**Blueprint source sync (2.10.0).** `BlueprintSyncTest.kt` is the `EntitySyncTest.kt` case list
retargeted one level up, ADMIN via `seededClient`, no waiver: create with `sourceUrl` (`GET
…/sync` `lastSyncedAt 0`), invalid URLs `400` on POST/PUT, sync overwrites and stamps
`updatedAt == lastSyncedAt` from a baseline equal to the sanitized, merged submitted request
without `sourceUrl`, `blueprint.synced` audited, `400` without a reference, `400` on
`document.sourceUrl`, a validation-failing sync answering `400` with the row byte-identical
afterward (no waiver), the **`hierarchyRelations` trio** (a body omitting the map keeps the
stored one on GET and in the baseline; a present map replaces it; an omitting body whose remote
dropped the named relation is `400` naming `hierarchyRelations.<k>`), `_team` extended via sync
(`system=true` audited) and `_team` renamed via sync `400`, a rename via sync cascading with
`renamedFrom`, a rename onto a taken identifier `409`, USER `403` on `POST …/sync` and fetch (200
on `GET …/sync`), `404` on unknown ids (even with a structurally invalid document), and the
import batch `sourceUrl` (D3 keep-vs-move, per-document `sourceUrl` `INVALID`, pass-2 restoration
carrying the FINAL document, `/import/check` parity). `UrlFetchTest` gains the
`POST /api/v1/blueprints/fetch` route through the same `UrlFetcherKey` seam (a fixture-server 200
audited `blueprint.fetched`, a loopback URL `400` audited `blueprint.fetch_blocked`, USER `403`
before any fetch). `SampleBlueprintsTest` and `IntegrationOntologyParityTest` are re-pinned in the
SAME commit as the checker change (`.claude/docs/port-data-model.md` "Computed-property health").
Frontend: `SyncBlueprintModal.test.tsx` (the `SyncEntityModal.test.tsx` case list minus
computed/blueprint-mismatch, plus the hierarchy-kept sync and both `["blueprints"]`/`["entities"]`
invalidations), `Blueprints.test.tsx` (the kebab-driven Last-sync states and the Sync item's
disabled/enabled wiring), `EditBlueprint.test.tsx` (the Source field and the header Sync action).
E2E stays at the loopback-refusal posture only — `blueprint-sync.spec.ts`: the
fetch→diff→overwrite happy path is deliberately server-/unit-tested, never run against external
...
**E2E scenarios (design artifacts).** The Playwright suite in `e2e/` is governed by `e2e/README.md` (run recipes, the parallel state-ownership rulebook, the coverage map). Every spec has a **natural-language scenario file** in `e2e/scenarios/` — versioned, deliberately non-executable design artifacts (actors, owned state, numbered user-level steps, expected outcomes; `## Scenario:` headings equal the `test()` titles verbatim). `e2e/scenarios/README.md` holds the format and the **compiler contract** — the house rules any human/agent/tool must satisfy when turning a scenario into spec code.

- **Applies when**: a new or behaviorally changed Playwright `test()` lands.
- **Requirement**: it ships, in the same commit, with its scenario file in `e2e/scenarios/` (headings matching its `test()` titles verbatim) and its one-line entry in `e2e/README.md`'s coverage map.
- **Reference**: `e2e/scenarios/README.md` (format + compiler contract).
- **Enforcement**: `cd e2e && npm run check:scenarios` (spec ↔ scenario-file existence and title/heading parity, both directions; spec ↔ coverage-map bullet, both directions, orphans included); `npm run typecheck` separately covers what Playwright's transpile-only TS handling never checks.
- **Exception**: `accessibility.spec.ts`'s parameterized titles are exempt from the title/heading parity check (the template-title skip in `scenarios/README.md`).
