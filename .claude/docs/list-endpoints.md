...
- Filtering: whitelisted equality filters on the field's own name plus `field[gte|gt|lte|lt]` bracket operators only where needed; strict `true`/`false` booleans; enums by string name; repeated key reserved for `IN` — the params with documented `IN` semantics today are the catalog list/graph's `kind` (any-of over the supported kinds), `labelValue` (any-of over the selected label's value), and the entity graph's `blueprint` (any-of over blueprint identifiers, case-insensitive, unknown identifiers ignored rather than `400` — an all-unknown list answers an empty graph), all three read via `repeatedValues`; on every other scalar key repetition stays a `400` (Lettuce's `singleValue` rule — never silent first-value-wins; unknown parameter *names* stay ignored by deliberate leniency) [API-LIST-004].
...
**Second paged consumer.** `GET /api/v1/entities` (Phase 2 of the Port data-model move — see
...
**The entity errors report (2.5.0).** `GET /api/v1/entities/errors` answers an unpaged report bounded by the workspace cap, the `GET /api/v1/files/errors` posture one level up — not a list endpoint either.
