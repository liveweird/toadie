...
| API-ERR-005 | `413` is declared only on the body-heavy catalog operations and the two ontology imports, though `RequestBodyLimit` (10 MiB, `plugins/Http.kt`) is installed globally, so every operation can in principle answer it | Deliberate (`CLAUDE.md` "Request body ceiling" — a memory-DoS backstop, not a per-operation business rule); adopt by adding `'413': { $ref: '#/components/responses/PayloadTooLarge' }` to every remaining body-taking operation if a consumer needs it declared everywhere |
