...
/**
 * The Entity graph/hierarchy pages' shared filter state (v1.25.0) — a blueprint multi-select
 * plus free-text search, the mirror of `useCatalogFileFilterState` scaled down to the two
 * params `GET /api/v1/entities/graph` declares. Persisted per view under
 * `toadie.viewSettings.<viewKey>.filter.*` (NOT in the URL — the catalog views' posture; the
 * Entities LIST's own `?blueprint=` param is unrelated). Only `q` debounces (300 ms); the
 * blueprint MultiSelect changes discretely.
 */
export function useEntityGraphFilterState(viewKey: string): {
  ...
  const activeFilterCount = (blueprints.length > 0 ? 1 : 0) + (q.trim() ? 1 : 0);

  return { values, activeFilterCount, controls: { blueprints, setBlueprints, q, setQ } };
}
