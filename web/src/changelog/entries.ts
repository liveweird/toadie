...
export const CHANGELOG: readonly ChangelogEntry[] = [
  {
    version: "2.11.1",
    date: "2026-09-20",
    en: `**Bounded database connections.** The server now keeps a fixed-size pool of PostgreSQL connections (20 by default, POSTGRES_POOL_MAX_SIZE) instead of opening a fresh one for every request — a burst of traffic can no longer exhaust the database's connection limit. Also from the September checkup: the Kubernetes rollout recreates the single app instance instead of briefly running two, the migration log no longer echoes the database URL, a malformed sign-out body is no longer logged, and the reference docs caught up with the last eight releases.`,
    pl: `**Ograniczona liczba połączeń z bazą.** Serwer utrzymuje teraz pulę połączeń z PostgreSQL o stałym rozmiarze (domyślnie 20, POSTGRES_POOL_MAX_SIZE) zamiast otwierać nowe dla każdego żądania — nagły ruch nie wyczerpie już limitu połączeń bazy. Ponadto z wrześniowego przeglądu: wdrożenie w Kubernetes odtwarza pojedynczą instancję aplikacji zamiast na chwilę uruchamiać dwie, log migracji nie wypisuje już adresu bazy, niepoprawne ciało wylogowania nie trafia do logu, a dokumentacja referencyjna nadrobiła ostatnie osiem wydań.`,
  },
  {
    version: "2.11.0",
    date: "2026-09-20",
