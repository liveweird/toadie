...
- Database connections go through one bounded R2DBC pool (`postgres.pool.*` in `application.yaml`: `POSTGRES_POOL_MAX_SIZE` 20, `POSTGRES_POOL_INITIAL_SIZE`, `POSTGRES_POOL_MAX_ACQUIRE_SECONDS`, `POSTGRES_POOL_MAX_IDLE_SECONDS`; every connection tagged `application_name = toadie`) — see `.claude/docs/persistence.md` "Connection pool".
...
- **`sample-data/`** — two independent commerce/payments demos. Start with the [sample-data guide](sample-data/README.md) for prerequisites, loading instructions, expected results, and cleanup. `sample-data/backstage/commerce-payments/catalog-info.yaml` is a 34-document Backstage catalog loaded through `/files/import`; its shapes and registry vocabulary are valid, while four intentional soft reference findings in two rows produce 32 `CREATED` and 2 `CREATED_WITH_FINDINGS`. `sample-data/port/commerce-payments/` is the Port demo: fifteen dependency-ordered baseline blueprints and 76 entities, loaded by its two `load.sh` scripts or `/ontology/import`, loaded with their public raw-GitHub source references by default (2.10.2 — `TOADIE_SOURCE_BASE`, empty for a source-less load). The Port model demonstrates the parallel `composition` and `deployment` hierarchies, and since 2.11.0 the `product` blueprint (the commercial offering, linked from systems via `system.products`), and since 2.13.0 the `dataset` blueprint plus the `api_adoption`/`dataset_adoption` relation entities (declared usage, the `workload` pattern), and twenty computed properties. Neither demo is seeded: migrations insert no catalog files or entity instances; V31 seeds only the protected `_team`/`_user` blueprints, which the Port sample extends.
...
**Toadie is moving from Backstage's fixed System Model to [Port.io](https://docs.port.io/context-lake/data-model/configure-data-model/)'s ontology in phases** — user-definable **blueprints** (typed properties, relations, mirror/calculation/aggregation properties, ownership) replace the seven predefined kinds, and **entities** are instances attached to a blueprint. Phase 1 (v1.23.0) is the `blueprints/` registry: create/edit/list/delete blueprint definitions in Port's native JSON shape, ADMIN-curated and readable by everyone, with nothing in the catalog/kinds machinery changed yet. Phase 2 (v1.24.0) is the `entities/` package: instances of blueprints — any authenticated user may create/read/update/delete any entity (the catalog-file shared-workspace rule, no admin gate), `properties` typed by the owning blueprint's `schema`, `relations` naming other entities of the target blueprints, and strict per-write validation (`entityFindings`) reused unchanged by every GET/list `findings` field so an entity left stale by a later blueprint edit is visibly flagged without a background job; `catalog/` itself is untouched by this phase. Phase 3 (v1.25.0): the Entity graph and Entity hierarchy pages in the Port world's Ontology section, driven by the Toadie-only `hierarchyRelations` (plural, v1.32.0) — a map of hierarchy identifier (active `HIERARCHY` dictionary value) to relation key, naming zero or more of the blueprint's own `many: false` relations as the parents for each hierarchy's tree; stored beside the Port document, so the Port document itself stays untouched; `GET /api/v1/entities/graph` renders entities and their relations together the way `catalog/Graph.kt` does one level down, with its own per-user layout persistence (V30) independent of the Backstage Graph page's. Phase 4 (v1.26.0): users and teams — Port's `_team`/`_user` SYSTEM blueprints seeded by V31 (protected, extendable) and `$team` ownership on every entity (the `team` field validated against `_team`, Inherited ownership computed from `ownership.path`, a `team` filter on the entity list/graph (since v1.30.0 matching the EFFECTIVE team, Inherited ownership included) and `$team` edges on the Entity graph); login accounts stay separate from `_user` entities, and ownership is informational — it never gates permissions. Phase 5 (v1.27.0): computed properties — `mirrorProperties`, `calculationProperties` (real jq 1.6 via `net.thisptr:jackson-jq` 1.3.0) and `aggregationProperties` are evaluated at entity READ time and merged into every GET/list/create response's `properties` (computed wins over a stale stored key; an unresolvable value is simply absent, never a finding), while `POST`/`PUT` still reject a computed property id as write input (`COMPUTED_PROPERTY`, unchanged since phase 2); evaluation runs OUTSIDE the database transaction (`entities/JqCalculation.kt`, `EntityComputed.kt`, `EntityAggregation.kt`, `AggregationQuery.kt`) so a pathological expression or a wide aggregation fan-out never pins a pooled connection or the entity write lock; since v1.29.0 each jq expression additionally runs on a bounded `entity-jq` worker pool under a per-expression deadline (default 500 ms), and an expression that misses it is quarantined until edited. Phase 6 (v1.28.0): ontology import and export — `POST /api/v1/blueprints/import`+`/import/check` (ADMIN) and `POST /api/v1/entities/import`+`/import/check` (any authenticated) accept a batch of up to 200 raw Port-shaped JSON documents, report-and-skip per row with ONE shared classification for the real run and its dry-run; a pure planner topologically orders each batch over its own sibling relation/aggregation/`team`/format-property references and defers what a document's first write cannot yet resolve into a second pass (an entity's MANDATORY reference cycle is rejected outright instead), and a `replaceExisting` switch turns an `EXISTS` row into a full `UPDATED` PUT (extending `_team`/`_user` too); the `/ontology/import` page (the Port world's Ontology nav) and a client-side JSON export (the whole blueprint registry from the Blueprints page; one chosen entity as Port's create-entity body from the Entities row menu, 2.8.0) completes the round trip — the exported shapes are exactly what the import accepts back unchanged. Phase 7 (v2.0.0): the **entity query language** — a read-only, openCypher-shaped subset (`MATCH`+, trailing `OPTIONAL MATCH`*, `WHERE`, `RETURN` of node variables, `LIMIT`) typed into the CodeMirror query bar the Entity graph and Entity hierarchy canvases share, sent as `query` on `GET /api/v1/entities/graph` and evaluated IN MEMORY over the whole active workspace (`entityquery/` — hand-written parser, validator with "did you mean" suggestions, level-set BFS evaluator under a cooperative 2-second budget, run OUTSIDE the read transactions on a dedicated bounded pool); the entities its `RETURN` binds become the shown set, intersected with the `blueprint`/`q`/`team` filters; a refused query is a `400` carrying positioned `diagnostics`, and `POST /api/v1/entities/query/check` is the editor's live check; since v2.1.0 a query can be SAVED (`/api/v1/entity-queries`, V35 — the Lenses model one level over: named, PRIVATE/PUBLIC, creator-only mutable, picked from the bar's header row on both canvases); since v2.2.0 the canvases GENERATE queries — a node's context menu on the Entity graph and the row menu's Query group on the Entity hierarchy emit Expand/Ancestors/Descendants/Owned-by templates (`web/src/utils/queryTemplates.ts`) into the bar. Phase 8 (v2.5.0): the Port-world **Errors report** (`GET /api/v1/entities/errors`, `/ontology/errors`) — the `catalog/Errors.kt` counterpart one level up, reporting stale entities, unresolved ownership, broken saved queries, computed-property health (a blueprint's mirror/aggregation/ownership reference into another blueprint gone stale, or a `calculationProperties` jq expression that fails to compile or sits quarantined on this instance), and, since 2.9.1 (entities) and 2.10.0 (blueprints), source-less entities and blueprints (`SOURCE_MISSING`, report-only) in one workspace-wide, report-only pass — see `.claude/docs/port-data-model.md` "Computed-property health". Since 2.9.0 entities carry the catalog's source reference & HTTP re-sync twin: an optional `sourceUrl` envelope column, `GET`/`POST /api/v1/entities/{id}/sync`, `POST /api/v1/entities/fetch`, and a batch `sourceUrl` on the entity import that stamps every stored row synced — strict, a copy failing `entityFindings` is refused and never stored. Since 2.10.0 blueprints gain the same envelope one level up — `sourceUrl`, `GET`/`POST /api/v1/blueprints/{id}/sync` (ADMIN), `POST /api/v1/blueprints/fetch` (ADMIN), and a batch `sourceUrl` on the blueprint import — with one difference: a sync that omits `hierarchyRelations` KEEPS the stored map rather than clearing it, since the Toadie-only extension has no Port-native representation to omit-by-absence against. Port source-sync correctness and its `/api/v1` compatibility guard are specified in `.claude/docs/persistence.md` ("Source-bound sync") and the two modal sections of `web/CLAUDE.md`.

**`.claude/docs/entity-query-language.md` is the language reference** (grammar, semantics, caps, rejected features, diagnostic codes). **`.claude/docs/port-data-model.md` is the local offline reference for Port's blueprint AND entity rules** (every property type/format/colour, the relation/mirror/calculation/aggregation/ownership grammars, meta-properties, the identifier-charset assumption, and — since phase 2 — the entity wire shape, its value-validation rule table, and the lifecycle rules) — consult it when designing any blueprint/entity feature instead of browsing, and update it when a rule is added. **`.claude/docs/ontology.md` is the baseline ontology itself** (v1.25.2): the fifteen blueprints under `sample-data/port/commerce-payments/blueprints/`, their Backstage round-trip contract and the design decisions — the model every future catalog feature assumes, pinned by `SampleBlueprintsTest`.
