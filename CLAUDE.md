...
- **`sample-data/`** — a 34-entity multi-document `catalog-info.yaml` (plus its README) speaking the V22-seeded registry vocabulary, loaded BY HAND through the Import page or `POST …/import` to exercise the app; and (v1.23.1) `sample-data/blueprints/` — eight Port-native blueprint JSON documents (`01-team.json` … `08-organization.json`, dependency-ordered) covering every property type/format/spec, all 14 enum colours, relation shapes, mirror/calculation/aggregation properties, and both ownership modes — and (v1.25.1) five of them (`domain`/`service`/`workload`/`deployment`/`incident`) carry a `hierarchyRelation`, forming the team → domain → service → workload → deployment tree with incidents nested under services — loaded BY HAND via `sample-data/blueprints/load.sh` (`POST …/api/v1/blueprints`; there is no import UI for blueprints); and (v1.24.1) `sample-data/entities/` — eight numbered JSON files of `POST …/api/v1/entities` bodies, one per sample blueprint in the same dependency order, ~32 entities showcasing every property/enum value and both `team` shapes, loaded BY HAND via `sample-data/entities/load.sh` after the blueprint set; and (v1.25.2) `sample-data/ontology/` — the eleven-blueprint BASELINE ontology the platform catalog is built on (`01-team.json` … `11-workload.json`, dependency-ordered, its own `load.sh`), an alternative to the showcase set (they share identifiers — load one or the other), designed in `.claude/docs/ontology.md`. Deliberately NOT seeded: no migration inserts catalog files, blueprints, or entities, so every environment still comes up with an empty workspace.
