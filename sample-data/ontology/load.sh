#!/usr/bin/env bash
# Loads (or, with --delete, removes) the baseline ontology at sample-data/ontology/*.json — the
# eleven-blueprint model the platform catalog is built on (.claude/docs/ontology.md) — against
# a running toadie instance, using the blueprint API (the sample-data/blueprints/load.sh idiom;
# there is no import UI for blueprints). Re-runnable: an already-loaded file is reported
# "exists, skipped", not an error. Do NOT load it on top of the showcase set: the two share
# identifiers (team, domain, service, environment, workload) and the second one in is skipped.
#
# Env:
